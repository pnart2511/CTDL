	...
	void nhapdanhsachgv(List L)
	{
		int i ;
		printf("Nhap Vao so luon giao vien:");
		Makenull_List(L);
		Node* current = NULL;
		for(int i = 0 ; i<n ; i++)
		{
			Node *newNode = (Node*)malloc(sizeof(Node));
			printf("Nhap thong tin cua giao vien\n",i+1);
			printf("Ho Ten:");
			getchar();
			fgets(newNode->Element.Hoten,sizeof(newNode->Element.Hoten).stdin);
			newNode->Element.Hoten[strcspn(newNode->Element.Hoten,"\n")] = '\0';
			printf("Nam Sinh: ");
			scanf("")
	...
	typedef struct DanhSachGiaoVien
	{
		Node* head;
	}
